#!/usr/bin/env python3

def hex_to_bytes(hex_string):
    return bytes.fromhex(hex_string)

def xor_bytes(a, b):
    return bytes(x ^ y for x, y in zip(a, b))

def solve_many_time_pad_attack():
    """
    Solve using the classic many-time pad attack:
    1. XOR ciphertexts together
    2. Look for results that are letters (indicating space XOR letter)
    3. Use this to gradually recover the key
    """
    
    # All ciphertexts including target
    all_hex = [
        "",
        "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f",
        "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb",
        "",
        "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070",
        "",
        "",
        "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3",
        "",
        "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83",
        # Target ciphertext
        "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
    ]
    
    # Convert to bytes
    ciphertexts = [hex_to_bytes(ct) for ct in all_hex]
    target = ciphertexts[-1]  # Target is the last one
    ciphertexts = ciphertexts[:-1]  # Remove target from the list
    
    min_len = min(len(ct) for ct in ciphertexts + [target])
    ciphertexts = [ct[:min_len] for ct in ciphertexts]
    target = target[:min_len]
    
    print(f"Working with {len(ciphertexts)} ciphertexts and 1 target of length {min_len}")
    
    # Step 1: Find space positions by XORing ciphertexts
    space_positions = set()
    
    for i in range(len(ciphertexts)):
        for j in range(i + 1, len(ciphertexts)):
            xor_result = xor_bytes(ciphertexts[i], ciphertexts[j])
            
            for pos, byte_val in enumerate(xor_result):
                # If result is alphabetic, one plaintext likely has space
                if ((65 <= byte_val <= 90) or (97 <= byte_val <= 122)):
                    space_positions.add(pos)
    
    print(f"Found {len(space_positions)} potential space positions")
    
    # Step 2: For each potential space position, try to recover key
    key = [None] * min_len
    
    for pos in sorted(space_positions):
        # Try each ciphertext having space at this position
        best_key = None
        best_score = 0
        
        for ct_idx, ct in enumerate(ciphertexts):
            candidate_key = ct[pos] ^ ord(' ')
            
            # Test this key byte against all ciphertexts and target
            score = 0
            valid_chars = []
            
            for test_ct in ciphertexts + [target]:
                if pos < len(test_ct):
                    decoded_char = test_ct[pos] ^ candidate_key
                    if 32 <= decoded_char <= 126:  # Printable ASCII
                        score += 1
                        valid_chars.append(chr(decoded_char))
                    else:
                        score -= 1
            
            # Prefer keys that produce reasonable English characters
            english_score = sum(1 for c in valid_chars if c.isalpha() or c.isspace() or c in '.,!?;:-()\'\"')
            total_score = score + english_score
            
            if total_score > best_score:
                best_score = total_score
                best_key = candidate_key
        
        if best_key is not None and best_score > len(ciphertexts) // 2:
            key[pos] = best_key
    
    # Step 3: Try to fill remaining positions using frequency analysis
    print("Filling gaps with frequency analysis...")
    
    # Most common English words and patterns
    common_patterns = [
        "the ", "and ", "that ", "have ", "for ", "not ", "with ", "you ", "this ", "but ",
        "his ", "from ", "they ", "she ", "her ", "been ", "than ", "its ", "who ", "oil ",
        "said", "each", "which", "their", "time", "will", "about", "would", "there", "could",
        "other", "after", "first", "into", "over", "think", "also", "your", "work", "life",
        "only", "can", "still", "should", "being", "now", "made", "before", "here", "through",
        "when", "much", "go", "me", "so", "these", "up", "out", "many", "then", "them",
        "what", "know", "take", "than", "them", "well", "were"
    ]
    
    for pattern in common_patterns:
        pattern_bytes = pattern.encode('ascii')
        
        # Try placing this pattern in each ciphertext
        for ct in ciphertexts:
            for start_pos in range(len(ct) - len(pattern_bytes) + 1):
                # Check if we can place this pattern here
                conflicts = False
                temp_key = []
                
                for i, p_byte in enumerate(pattern_bytes):
                    pos = start_pos + i
                    k = ct[pos] ^ p_byte
                    
                    if key[pos] is not None and key[pos] != k:
                        conflicts = True
                        break
                    temp_key.append(k)
                
                if not conflicts:
                    # Verify this key segment works well for other texts
                    validity_score = 0
                    
                    for other_ct in ciphertexts + [target]:
                        if other_ct == ct:
                            continue
                        
                        segment_valid = True
                        for i, k in enumerate(temp_key):
                            pos = start_pos + i
                            if pos < len(other_ct):
                                decoded = other_ct[pos] ^ k
                                if not (32 <= decoded <= 126):
                                    segment_valid = False
                                    break
                        
                        if segment_valid:
                            validity_score += 1
                    
                    # If this pattern works well, apply it
                    if validity_score >= len(ciphertexts) // 2:
                        for i, k in enumerate(temp_key):
                            pos = start_pos + i
                            if key[pos] is None:
                                key[pos] = k
                                print(f"Applied pattern '{pattern}' at position {start_pos}")
    
    # Step 4: Decode the target
    result = []
    for i, byte_val in enumerate(target):
        if key[i] is not None:
            decoded = byte_val ^ key[i]
            if 32 <= decoded <= 126:
                result.append(chr(decoded))
            else:
                result.append('?')
        else:
            result.append('?')
    
    decoded_message = ''.join(result)
    print(f"\nDecoded target: '{decoded_message}'")
    
    # Extract readable message
    import re
    
    # Try to extract the most readable continuous sequence
    words = decoded_message.split()
    readable_words = []
    
    for word in words:
        # Remove leading/trailing punctuation and check if mostly alphabetic
        clean_word = re.sub(r'^[^a-zA-Z]*|[^a-zA-Z]*$', '', word)
        if len(clean_word) >= 2 and sum(c.isalpha() for c in clean_word) >= len(clean_word) * 0.7:
            readable_words.append(word)
    
    if readable_words:
        secret = ' '.join(readable_words)
        print(f"Extracted secret: '{secret}'")
        return secret
    
    # Alternative: look for longest alphabetic sequence
    alpha_sequences = re.findall(r'[a-zA-Z .,!?;:\'-]{4,}', decoded_message)
    if alpha_sequences:
        secret = max(alpha_sequences, key=len).strip()
        print(f"Longest readable sequence: '{secret}'")
        return secret
    
    return decoded_message.strip()

if __name__ == "__main__":
    secret = solve_many_time_pad_attack()
    print(f"\n=== FINAL ANSWER ===")
    print(f"Secret message: {secret}")